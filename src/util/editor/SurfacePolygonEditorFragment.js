...
define([
    ...
    function (BaseSurfaceEditorFragment,
        ...
        SurfacePolygonEditorFragment.prototype.reshape = function (shape,
            ...
            if (controlPoint.userProperties.purpose === ShapeEditorConstants.ROTATION) {
                ...
                shape.stateKeyInvalid = true;
                    ...
                    controlPoint.userProperties.purpose === ShapeEditorConstants.SHADOW) {
                var index = controlPoint.userProperties.index;
                var boundaries = shape.boundaries;
                var lenBoundaries = boundaries.length;
                var picked = this.getRingAndLocationIndex(shape, index);

                if (secondaryBehavior) {
                    if (picked.ringIndex !== -1) {
                        var ring = boundaries[picked.ringIndex];
                        if (ring.length > 3) {
                            ring.splice(picked.locationIndex, 1);
                        } else if (lenBoundaries > 2) {
                            boundaries.splice(picked.ringIndex, 1);
                        }
                    } else {
                    ...
                    if (controlPoint.userProperties.purpose === ShapeEditorConstants.SHADOW) {
                        if (picked.ringIndex !== -1) {
                            this.moveLocation(globe, controlPoint, previousPosition, currentPosition, boundaries[picked.ringIndex][picked.locationIndex + 1]);
                        } else {
                            this.moveLocation(globe, controlPoint, previousPosition, currentPosition, locations[index + 1]);
                        }
                    } else {
        ...
        // Internal use only.
        SurfacePolygonEditorFragment.prototype.getRingAndLocationIndex = function (shape, index) {
            var locations = shape.boundaries;
            var lenBoundaries = locations.length;
            var result = {
                locationIndex: -1,
                ringIndex: -1
            };

            if (lenBoundaries > 0 && Array.isArray(shape.boundaries[0])) {
                var locationOffset = 0;

                for (var i = 0; i < lenBoundaries && result.ringIndex == -1; i++) {
                    var len = locations[i].length;
                    if (locationOffset + len > index) {
                        result.ringIndex = i;
                        result.locationIndex = index - locationOffset;
                    }
                    locationOffset += len;
                }
            }
        ...
        // Internal use only.
        SurfacePolygonEditorFragment.prototype.convertShadowControlPoint = function (shape, globe, index, position) {
            var locations = shape.boundaries;
            var picked = this.getRingAndLocationIndex(shape, index);

            if (picked.ringIndex !== -1) {
                locations[picked.ringIndex].splice(picked.locationIndex + 1, 0, position);
            } else {
                locations.splice(index + 1, 0, position);
            }
